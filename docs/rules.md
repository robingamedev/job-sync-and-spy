...
- **Schema Integrity (DO NOT FORK THE SCHEMA):** The `frontend/prisma/schema.prisma` is a direct clone from the upstream JobSync repository. **AI Agents are strictly forbidden from creating custom Prisma models** (e.g., no `PocketSpySettings`, no `SystemLog`). Any new scraper features must map to the native JobSync tables (like `Automation`, `AutomationRun`, and `UserSettings`) so the scraping logic can be cleanly contributed upstream as a "headless worker."
